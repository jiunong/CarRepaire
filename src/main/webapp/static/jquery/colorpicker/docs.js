...
function getColorOptions(divname){
var colorOptions={
  allowEmpty:true,
    color: "#000000",
    showInput: true,
 ...
 * 修改图中线
 */
function hichartsColor(){
	var reg1=/^columnColor{1}\d{1,2}_{1}container{1}\d{1,2}$/;
	if(divname=="tiColor" ||divname=="ti2Color"){
		updateTitleColor();
	}else if(divname=="customcolor"){

	}
	else if(divname=="xBandColor"){
		addXPlotBand();
	}
	else if(divname=="yBandColor"){
		addYPlotBand();
	}
	else if(divname=="xlineColor"){
		addXPlotLine();
	}
	else if(divname=="ylineColor"){
		addYPlotLine();
	}else if(divname=="mapGreaterColor"){
		var max= getValue("#mapGreaterValue");
		showCondition(1,max,null);
		
	}else if(divname=="mapLessColor"){
		var min= getValue("#mapLessValue");
		showCondition(2,null,min);
		
	}else if(divname=="mapScopeColor"){
		var max= getValue("#ScopeValue1");
		var min= getValue("#ScopeValue2");
		showCondition(3,max,min);
		
	}else if(reg1.test(divname)){
		//columnColor0_container1;for highcharts
		var val=divname.split("_");
		var name=val[0].replace("columnColor","");
		modifyColumnColor(val[1],name,hexColor);
	}else if(divname == "backgroundline" || divname == "highlightBigColors" || divname == "fontColor" ){
