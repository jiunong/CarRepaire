...
//添加
function add(){
	var url =  "user/edit.do?tag=1";
	var name = '添加系统用户信息';                           //网页名称，可为空;  
...
//删除多行
function deleteRow(){
	 var tables = $('#user').bootstrapTable('getAllSelections');
	 var ids = "";
   if(tables.length==0){
        layer.alert('您至少选择一行！');
  		return false;
...
if(tables.length==0){
    ...
    var id = tables[0].uId;
	  var url =  "user/edit.do?id=" + id;
	  var name = '修改系统用户信息';                           //网页名称，可为空;  
	  var iWidth = '900';                         //弹出窗口的宽度;  
	  var iHeight = '550';                        //弹出窗口的高度;  
	  //window.screen.height获得屏幕的高，window.screen.width获得屏幕的宽  
	  var iTop = (window.screen.height-30-iHeight)/2;       //获得窗口的垂直位置;  
